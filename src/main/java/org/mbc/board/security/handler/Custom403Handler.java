...
@Log4j2 // Custom403Handler 는 CustomSecurityConfig 클래스에서 활용된다.
public class Custom403Handler implements AccessDeniedHandler {
    //                                   AccessDeniedHandler 내장된 예외처리용 클래스 (재정의)
    // 현재 사용자의 권한이 없거나
    // 특정 조건이 맞지 않아 예외가 발생하는 403 처리용 클래스
    ...
    public void handle(HttpServletRequest request, HttpServletResponse response, AccessDeniedException accessDeniedException) throws IOException, ServletException {
        ...
        log.info("403 예외 발생 중!");
        
        response.setStatus(HttpStatus.FORBIDDEN.value());   // 현재 예외 상태값 저장
        String contentType = request.getHeader("content-type"); // json 여부
        boolean jsonRequest = contentType.startsWith("application/json");
        log.info("isJSON : " + jsonRequest);  // jsno 일 때 처리 끝

        // 일반 요청 시
        if(!jsonRequest) {
            response.sendRedirect("/member/login?error=ACCESS_DENIED");
        }   // 4032 예외 발생시 로그인 화면으로 이동하고 error 처리
