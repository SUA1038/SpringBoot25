...
public class SampleController {
    ...
    @GetMapping("ex/ex1")   // http://192.168.111.105/ex/ex1 -> /resources/template/ex/ex1.html
    ...
    public void ex2(Model model){
        ...
        List<String> strList = IntStream.range(1,10) // 1~10까지정수를 생성한다.
                .mapToObj(i -> "데이터"+i)
        ...
        model.addAttribute("list", strList); // 66행에 만들어진 객체 값
        model.addAttribute("map", map);     // 70행에 만들어진 객체 값
        model.addAttribute("dto", sampleDTO); // 74행에 만들어진 객체 값

        // 리턴타입이 void 임으로 /resources/template/ex/ex2.html
    }

    @GetMapping("/ex/ex3") // http://192.168.111.105/ex/ex3 -> /resources/template/ex/ex3.html
